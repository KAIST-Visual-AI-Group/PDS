...
@dataclass
class PDSDataParserConfig(DataParserConfig):
    """PDS dataset config"""

    _target: Type = field(default_factory=lambda: PDSDataParser)
...
@dataclass
class PDSDataParser(DataParser):
    """PDS DatasetParser"""

    config: PDSDataParserConfig
    downscale_factor: Optional[int] = None
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in frames:
            ...
            if "first_stage_file_path" in frame:
                first_stage_filepath = Path(frame["first_stage_file_path"])
                first_stage_fname = data_dir / first_stage_filepath
                assert first_stage_fname, f"{first_stage_fname}"
                first_stage_filenames.append(first_stage_fname)
        ...
        assert len(first_stage_filenames) == 0 or (
            len(first_stage_filenames) == len(image_filenames)
        )
        ...
        first_stage_filenames = [first_stage_filenames[i] for i in indices] if len(first_stage_filenames) > 0 else []
        ...
        dataparser_outputs = DataparserOutputs(
            ...
            metadata={
                ...
                "first_stage_filenames": first_stage_filenames if len(first_stage_filenames) > 0 else None,
            },
        )
